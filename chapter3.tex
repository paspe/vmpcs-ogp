...
During the initialization of our DRAKVUF plugin, we read the \ac{SACL}s we have created for hthe \ac{VM} to be protected. At this point of the research we were limited to small lists, so the initial implementation for storing and searching through the \ac{SACL}s is two linked lists and two arrays of linked lists, as explained below. Figure~\ref{fig:sacl} depicts the structure created for storing each entry.
...
\par After we have retrieved the pathname, we then check for the system call that triggered the VM-exit event. The calls are handled as follows: 

\textit{sys\_open} with \textit{sys\_openat}, \textit{sys\_rename} with \textit{sys\_renameat} and \textit{sys\_renameat2}, and \textit{sys\_unlink} with \textit{sys\_unlinkat}.
