...
\par The above memory structure is used for both protected folders and protected files. Moreover, we create two of each; one for the \codeft{root} user and one for the rest. Searching in the \acp{SACL} is done with the pathname of the file being accessed as the hash-table key. The return value is a pointer to the structure of table \ref{tbl:sacl_layout}. 
