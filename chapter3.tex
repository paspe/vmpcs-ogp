...
\par In the case of \textit{sys\_unlink} and \textit{sys\_unlinkat} the procedure is straightforward. Once we look in the \ac{SACL}, if there is an entry, we verufy that the user or group deleting the file, has write permissions. If that is true, the callback function returns control to the \ac{VM} to resume execution. If permissions do not match, the pointer to the filename string is modified to \textit{NULL}, so that the system call, after the \ac{VM} resumes execution, fails (figure~\ref{fig:unlink}).
...
\par The search for a match in the \ac{SACL}s is performed in two steps in all cases. Once to go through protected folders and once to go through individually protected files.